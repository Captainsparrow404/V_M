...
class PassAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path(
                'api/passes/<int:pass_id>/print/',
                self.admin_site.admin_view(self.print_pass),
                name='api-print-pass'
            ),
            path(
                'api/passes/<int:pass_id>/upload/',
                self.admin_site.admin_view(self.upload_pass),
                name='api-upload-pass'
            ),
        ]
    ...
    def print_pass(self, request, pass_id):
        try:
            pass_obj = Pass.objects.get(pk=pass_id)
            ...
            context = {
                ...
                'visit_date': pass_obj.visit_date.strftime('%d-%m-%Y'),
                'num_persons': pass_obj.num_persons,
                'accommodation_date': pass_obj.accommodation_date.strftime('%d-%m-%Y') if pass_obj.accommodation_date else None,
                'darshan_date': pass_obj.darshan_date.strftime('%d-%m-%Y') if pass_obj.darshan_date else None,
                'darshan_type': pass_obj.darshan_type,
            }

            # Render the print template
            html_content = render_to_string('passes/print_pass_content.html', context)
            return HttpResponse(html_content)

        except Pass.DoesNotExist:
            return JsonResponse({'error': 'Pass not found'}, status=404)
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)

    @csrf_exempt
    def upload_pass(self, request, pass_id):
        if request.method != 'POST':
            return JsonResponse({'error': 'Invalid request method'}, status=405)
        ...
        try:
            ...
            if 'pass_pdf' not in request.FILES:
                return JsonResponse({'error': 'No PDF file uploaded'}, status=400)

            pdf_file = request.FILES['pass_pdf']
            ...
            msg = MIMEMultipart()
            msg['Subject'] = 'Temple Pass Approved'
            msg['From'] = settings.EMAIL_HOST_USER
            msg['To'] = pass_obj.email
            ...
            # Render email template
            html_content = render_to_string('passes/email_templates/pass_approved.html', context)
            msg.attach(MIMEText(html_content, 'html'))
            ...
            try:
                ...
                return JsonResponse({
                    'status': 'success',
                    'message': 'Pass approved and email sent successfully'
                })
            except Exception as e:
